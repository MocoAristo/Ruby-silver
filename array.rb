...
# 配列の変換（全て!がつくと破壊的メソッド）
# uniq      : 重複した要素を1つに圧縮する
# compact   : 要素のnilを取り除く
# reverse   : 配列を順序を逆にする
# flatten   : 多重配列を1次の配列に平滑化する
# collect   : 要素ごとにブロックを渡し、評価結果の配列を返す
# map       : collectと同じ
# shuffle   : 要素の順をランダムに変更する

array = [1,2,3,1,2,3,4,5,nil, [1,2,[3,4]]]
p array.uniq                    #=> [1, 2, 3, 4, 5, nil, [1, 2, [3, 4]]]
p array.compact                 #=> [1, 2, 3, 1, 2, 3, 4, 5, [1, 2, [3, 4]]]
p array.reverse                 #=> [[1, 2, [3, 4]], nil, 5, 4, 3, 2, 1, 3, 2, 1]
p array.flatten                 #=> [1, 2, 3, 1, 2, 3, 4, 5, nil, 1, 2, 3, 4]

array = [1,2,3,4,5,6]
p array.collect {|i| i *2 }     #=> [2, 4, 6, 8, 10, 12]
p array.map{|i| i**2}           #=> [1, 4, 9, 16, 25, 36]
p array.shuffle                 #=> [6, 1, 3, 2, 5, 4]


# 配列の組み合わせ
# product     : 2つの配列の1要素ずつの組み合わせを返す
# zip         : 2つの配列の1要素ずつインデックス順に格納した配列を返す
# *           : 引数の数だけ配列を繰り返す

array = [1,2,3]
p array.product(["a","b"])          #=> [[1, "a"], [1, "b"], [2, "a"], [2, "b"], [3, "a"], [3, "b"]]
p array.zip(["a","b"])              #=> [[1, "a"], [2, "b"], [3, nil]]
p array.zip(["a","b"],["x","y"])    #=> [[1, "a", "x"], [2, "b", "y"], [3, nil, nil]]
p array*3                           #=> [1, 2, 3, 1, 2, 3, 1, 2, 3]